...
``walker.services.fortnight.resolve_to_real_codes`` and in the frontend's
...
from walker.services.fortnight import FortnightGrid, FortnightRow, resolve_to_real_codes
...
def test_fixture_matches_backend_resolution(session: Session) -> None:
    ...
    grid = FortnightGrid(start=date(2026, 7, 1), end=date(2026, 7, 15), rows=rows)
