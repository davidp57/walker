...
def test_fortnight_relative_start_anchor_no_offset() -> None:
    # Current due 2026-07-01 (first fortnight); next fortnight starts 2026-07-16.
    rule = FortnightRelativeRule(anchor="start", offset_days=0)
...
def test_fortnight_relative_end_anchor_no_offset() -> None:
    # Current due in the fortnight ending 2026-07-15; next fortnight ends 2026-07-31.
    rule = FortnightRelativeRule(anchor="end", offset_days=0)
...
def test_fortnight_relative_end_anchor_last_working_day_before_end() -> None:
    # 2026-07-31 is a Friday -> last working day before/at fortnight end with offset -1 lands
    # on the last workday strictly before the boundary given Mon-Fri rhythm.
    rule = FortnightRelativeRule(anchor="end", offset_days=-1)

    result = next_due_date(rule, current_due=date(2026, 6, 15), workdays=MON_FRI, absences=set())

    # Fortnight ending 2026-06-30 (Tuesday, a workday) -> 1 working day before = 2026-06-29 (Monday).
    assert result == date(2026, 6, 29)


def test_fortnight_relative_end_anchor_skips_weekend_to_find_last_working_day() -> None:
    # Fortnight ending 2026-08-15 is a Saturday; offset 0 snaps back to the last working day.
    rule = FortnightRelativeRule(anchor="end", offset_days=0)
...
def test_fortnight_relative_end_anchor_skips_absence() -> None:
    # Fortnight ending 2026-06-30 (Tuesday, workday) with offset -1: 1 working day back would be
...
def test_fortnight_relative_start_anchor_positive_offset() -> None:
    # Next fortnight starts 2026-07-16 (Thursday); offset +1 working day -> 2026-07-17 (Friday).
    rule = FortnightRelativeRule(anchor="start", offset_days=1)
...
def test_fortnight_relative_from_second_half_rolls_to_next_month_start() -> None:
    rule = FortnightRelativeRule(anchor="start", offset_days=0)
...
def test_fortnight_relative_rejects_bad_anchor() -> None:
    with pytest.raises(ValueError, match="anchor must be 'start' or 'end'"):
        FortnightRelativeRule(anchor="middle", offset_days=0)  # type: ignore[arg-type]
